...
'''debug'''
sys.path.append('/home/luca/ros/devel/lib/python2.7/dist-packages')
sys.path.append('/opt/ros/melodic/lib/python2.7/dist-packages:')
sys.path.append('/usr/bin:/usr/local/lib/python3.6/dist-packages')
...
sys.path.append('/home/luca/ros/devel/share/')
sys.path.append('/home/luca/ros/src/myrobot/jsk_common_msgs/')
...
class DialogflowClient(object):
    def __init__(self, language_code='it-IT', last_contexts=None):
        ...
        # se il flag PLAY_GOOGLE_AUDIO e' attivo creo l'output audio
        ...
        rospy.Subscriber("/Tablet/voice", SpeechRecognitionCandidates, self._AndroidVoice2text_cb)
        ...
        print("\n\n==USA L'APP ROS VOICE RECOGNITION SU ANDROID PER CONVERTIRE LA VOCE IN TESTO ==\n\n")
    ...
    def _elabora_comandi_robot(self,final_result):
        '''
        #Elaboro il comando se l'intent non e' vuoto
        '''
        if (final_result.intent.display_name != ''):
            if (final_result.intent.display_name == 'cmd_repeat'):
                final_result = self.last_response_query_result
            self.myCmdParser.command_parser(final_result.intent.display_name,final_result.parameters )
            # Memorizzo lintent per eventuale comando repeat
            self.last_response_query_result = final_result
        else:
            print("\n\n### Nulla da elaborare ###\n\n")
    ...
    def _AndroidVoice2text_cb(self, voice):
        # Prendo la prima trascrizione (la piu probabile)
